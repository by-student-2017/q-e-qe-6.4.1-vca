...
! where
!   V_{loc)^{(vca)} = x V_{loc}^{(1)} + 0.0 V_{loc}^{(2)}
! and
!   V_{nl)^{(vca)} = \sum_{ij} |\beta^{(1)}_i> x D^{(1)}_{ij} <\beta^{(1)}_j|
!                  + \sum_{ij} |\beta^{(2)}_i> 0.0D^{(2)}_{ij} <\beta^{{2)}_j|
! where
...
PROGRAM virtual_test
  ...
  IF (ionode) THEN
     ...
     DO is=1,2
       ...
       filein(is) = "1.UPF"
       PRINT '(''  Input PP file # '',i2,'' in UPF format > '',a5,$)', is , filein(is)
     ...
     !
     PRINT '('' New Pseudo = x '',a,'' + 0.0 '',a)', (trim(filein(is)), is=1,2)
   10 CONTINUE
...
SUBROUTINE compute_virtual(x, filein, upf, upf_vca)
  ...
  upf_vca%comment    = 'Pseudo = x '//trim(filein(1))//&
                   ' + 0.0 '//trim(filein(2))//', with x='//xlabel
  upf_vca%psd = "Xx"
  upf_vca%typ = "NC"
  ...
  upf_zp    =  x * upf(1)%zp + 0.d0 * upf(2)%zp
  !
  ...
  IF (interpolate) THEN
     ...
     upf_rho_atc(1:upf_mesh) =    x     * upf(1)%rho_atc(1:upf_mesh) + &
                            0.d0 * aux1(1,1:upf_mesh)
     WRITE (*,*) " done"
  ELSE
     upf_rho_atc(1:upf_mesh) =    x     * upf(1)%rho_atc(1:upf_mesh) + &
                            0.d0 * upf(2)%rho_atc(1:upf_mesh)
  ENDIF
  !
  ...
  DO i=1,upf(2)%nbeta
     DO j=1,upf(2)%nbeta
        upf_dion(upf(1)%nbeta+i, upf(1)%nbeta+j) = 0.d0 * upf(2)%dion(i,j)
     ENDDO
  ENDDO
  !
  ...
  IF (matches(upf_vca%typ, "USPP")) THEN
    ...
    DO i=1,upf(2)%nbeta
       DO j=i,upf(2)%nbeta
          ...
          upf_qqq(upf(1)%nbeta+i,upf(1)%nbeta+j) = 0.d0 * upf(2)%qqq(i,j)
          ...
          IF( allocated(upf_qfuncl) ) THEN
             ...
             DO l=abs(l1-l2), l1+l2
                IF (interpolate) THEN
                   ...
                   CALL dosplineint( upf(2)%r(1:upf(2)%mesh), aux2, upf_r(1:upf_mesh), aux1 )
                   upf_qfuncl(1:upf_mesh, ijv2, l) = 0.d0 * aux1(1,1:upf_mesh)
                   WRITE (*,*) " done"
                ELSE
                   upf_qfuncl(1:upf_mesh, ijv2, l) = 0.d0 * upf(2)%qfuncl(1:upf_mesh,ijv,l)
          ...
          ELSE
             IF (interpolate) THEN
                ...
                CALL dosplineint( upf(2)%r(1:upf(2)%mesh), aux2, upf_r(1:upf_mesh), aux1 )
                upf_qfunc(1:upf_mesh, ijv2) = 0.d0 * aux1(1,1:upf_mesh)
             ELSE
                upf_qfunc(1:upf_mesh, ijv2) = 0.d0 * upf(2)%qfunc(1:upf_mesh,ijv)
             ENDIF
          ENDIF
          ! ! upf_qfunc(1:upf_mesh, upf(1)%nbeta+i, upf(1)%nbeta+j) = 0.d0 * qfunc(1:upf_mesh,i,j,2)
       ENDDO
    ENDDO
    !
       ...
       DO i=1,upf(2)%nbeta
          DO j=1,upf(2)%nbeta
             upf_qfcoef(1:upf_nqf,1:upf_nqlc,upf(1)%nbeta+i,upf(1)%nbeta+j) = &
                 0.d0 * upf(2)%qfcoef(1:upf_nqf,1:upf_nqlc,i,j)
          ENDDO
       ENDDO
    ENDIF
    !
